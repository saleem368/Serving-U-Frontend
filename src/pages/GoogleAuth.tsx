    ...
    if (window.location.hash) {
      ...
      if (accessToken) {
        ...
        fetch('https://www.googleapis.com/oauth2/v2/userinfo', {
          ...
          .then((res) => res.json())
          .then(async (profile) => {
            // Send to backend for JWT and profile info
            const localProfile = getProfileFromLocalStorage();
            const response = await fetch(`${API_BASE}/api/google-auth/google`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ email: profile.email, name: localProfile.name || profile.name, phone: localProfile.phone, address: localProfile.address }),
            });
            ...
            if (data.token) {
              ...
              // Save profile info from backend
              localStorage.setItem('userProfile', JSON.stringify({ name: data.name || '', phone: data.phone || '', address: data.address || '' }));
              navigate('/');
            }
          });
      }
